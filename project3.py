# Sololearn: Python Data Structure Code Project 3: Revenue Growth Analysis in a Ticket Office
...
# Representing ticket price with children < 18, and adults >= 18.
price = 0

# Representing ticket price of children < newAge, and adults >= newAge.
newPrice = 0

# Looping through the values (ie: age) of each key in the dictionary, to add to the price 5 or 20 depending on the age current limit.
for i in data.values():
   if i >= 18:
     price += 20
   else:
      price += 5
# Looping through the values (ie: age) of each key in the dictionary, to add to the price 5 or 20 depending on the newAge limit.
for i in data.values():
    if i >= age:
        newPrice += 20
    else: 
...
# Calculating the revenue, and printing it as an integer.
rev = ((newPrice  - price ) / price ) * 100
print(int(rev),"%")
